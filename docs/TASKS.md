...
- **Notes:** This follow-up is only needed because T0031 could not generate a PDF in the current Windows environment. After T0034, run this only after T0035 has either revised the report with the T0033 diagnosis or explicitly decided to leave the T0031 LaTeX artifact frozen. Do not rerun Noxim, change report claims, or modify simulator source while generating the PDF.
...
- **Files likely to change:** `docs/FINAL_REPORT_DRAFT.md`, `final_report/main.tex`, `docs/TASKS.md`, `docs/PROGRESS.md`, `docs/VALIDATION.md`, and `docs/PROMPTS.md`; `docs/DECISIONS.md` only if another durable report policy decision is made.
...
- **Validation method or limitation:** Use documentation/source consistency checks only. Run `git diff --check`. If LaTeX tools are present, optionally run a non-claim-changing compile check and record the generated PDF path or exact blocker; otherwise leave PDF generation to T0032.
- **Recommended prompt:** `Start task T0035: Revise Final Report with T0033 Diagnosis. Before starting, read AGENTS.md, docs/PROGRESS.md, docs/TASKS.md, docs/ROADMAP.md, docs/ARCHITECTURE.md, docs/VALIDATION.md, docs/DECISIONS.md, docs/PROMPTS.md, docs/FINAL_REPORT_DRAFT.md, and final_report/main.tex. Continue on the existing branch. Do not create or switch branches. Update docs/FINAL_REPORT_DRAFT.md and final_report/main.tex only to incorporate the T0033 diagnosis as explanatory, claim-safe provenance. Preserve all measured values, blank cells, limitations, assumptions, blockers, bibliography choices, IEEE style, and descriptive-only result language. Do not modify simulator source, helper source, routing logic, topology behavior, LUT behavior, traffic semantics, metrics semantics, or generated final-sweep artifacts. Do not rerun simulations, rebuild Noxim, regenerate final-sweep outputs, or use ./regression.sh --update. Run git diff --check and record whether PDF compilation was attempted or blocked. Update the tracking docs and report created files, modified files, source-code status, validation, current phase, next task, full next prompt, branch guidance, commit message, and unknowns or blockers.`
