...
- **Consequences:** T0044 may implement these semantics without reinterpreting existing fixed-window artifacts. T0026/T0027/T0028 and T0042 remain historical fixed-window/exploratory artifacts. Future eventual-delivery claims remain blocked until the opt-in mode is implemented, validated with targeted smokes, and used to create new versioned artifacts. Standard `XY`, `DEFT`, VN transition restrictions, VL fault injection semantics, LUT schema/use path, topology behavior, existing traffic semantics, existing metrics exports, and existing runner/analysis defaults must remain unchanged outside the opt-in mode.
