...
\begin{abstract}
This project implements and evaluates a Noxim-based \texttt{DEFT\_2\_5D} simulator extension for studying DeFT-style routing in a 2.5D chiplet network. The implementation models four chiplets on an active interposer, a deterministic physical Vertical Link (VL) inventory, startup permanent VL faults, DeFT virtual-network assignment rules, movement-transition filtering, schema-v1 offline VL lookup tables, explicit \texttt{XY} baseline modes, synthetic traffic profiles, machine-readable metrics export, and traceable experiment and analysis helpers. The final fixed-window sweep completed 150 simulator runs across routing modes, traffic profiles, fault masks, and seeds. The generated fixed-window results are valid for descriptive report support only. The measured data set contains empty and partial injection cells, including 54 completed runs with zero packets injected after the warm-up boundary. Therefore, this report presents finite-window measurements with coverage counts and limitations, while using the T0033 diagnosis only to explain the \texttt{XY} blank-cell provenance and avoiding unsupported performance conclusions. A later opt-in drain-mode validation set completed all 95 documented \texttt{DEFT} cases, and a separate \texttt{INTERPOSER\_AWARE\_XY} versus \texttt{DEFT} drain comparison completed 68 matched rows in both modes while leaving 27 IA-XY timeout or non-100\% rows as descriptive-only evidence.
\end{abstract}
...
T0033 diagnosed the current standard \texttt{XY} implementation as a limited control baseline on \texttt{DEFT\_2\_5D}, not as an interposer-aware unrestricted inter-chiplet baseline. The existing \texttt{XY} route path selects only cardinal directions from the global footprint. It does not select VL, hub, interposer, or chiplet-layer phases. On \texttt{DEFT\_2\_5D}, unrestricted inter-chiplet routes require VL/hub/interposer traversal, so standard \texttt{XY} can select chiplet-layer movement that has no physical cross-chiplet cardinal link.
...
T0033 also showed that a post-injection drain/source-cutoff policy would be needed for eventual-delivery analysis. That policy would not by itself fix standard \texttt{XY} topology incompatibility on unrestricted inter-chiplet traffic, because standard \texttt{XY} still has no VL/hub/interposer route phase.

T0056 and T0053 later used the opt-in source-cutoff plus drain mode for deterministic fixture matrices. T0056 used \texttt{DEFT} only. T0053 used \texttt{DEFT} and \texttt{INTERPOSER\_AWARE\_XY} only, with the same 19 fixture definitions as T0056. These artifacts are not fixed-window final sweeps and are not substitutes for the T0026/T0027/T0028 tables.
...
\textbf{Assumption:} T0033 warm-up-0 diagnostics are traceability evidence that the traffic sources were not empty. They are not a replacement final performance data set.

\textbf{Blocked:} The historical T0026/T0027/T0028 fixed-window runner policy does not evaluate eventual delivery after a post-injection drain phase.
...
T0033 is additional diagnosis provenance only. It explains the source of the \texttt{XY} blank cells by inspecting the runner warm-up policy, injected-packet accounting, standard \texttt{XY} routing, and \texttt{DEFT\_2\_5D} topology wiring. It does not add new final performance rows, does not replace T0026/T0027/T0028, and does not change simulator behavior.

Additional post-fix drain-mode provenance is traceable through T0056 at \path{external/noxim/other/generated/t0056_deft_post_fix_reachability_v1/} and T0053 at \path{external/noxim/other/generated/t0053_drain_iaxy_deft_comparison_v1/}. T0056 validation established a 95-row \texttt{DEFT} drain-mode matrix over 19 deterministic fixtures and the accepted physical fault-mask ladder. All rows returned code 0, stopped with \texttt{drain\_completed}, and passed with exact measured packet/flit delivery. T0053 validation established a 190-row drain-mode comparison matrix over \texttt{DEFT} and \texttt{INTERPOSER\_AWARE\_XY}. All \texttt{DEFT} rows passed. IA-XY passed 68 rows and timed out in 27 rows. The denominator-safe comparison table marks 68 matched rows as \texttt{complete\_delivery\_both\_modes} and 27 rows as \texttt{descriptive\_only\_timeout\_or\_non100}.

\section{Results and Evaluation}

The results in this section are assembled from the reviewed Markdown draft and the T0028/T0027/T0026 report-support artifacts. They are finite-window descriptive measurements. Blank cells are intentional and indicate that the supporting denominator is absent. The T0026/T0027/T0028 artifact chain remains the only final report-support data set used in the fixed-window tables below. T0056/T0053 drain-mode artifacts are summarized separately in Section~\ref{sec:drain_mode_results}. Following the final-report instructions, this section separates measured outputs, comparison readiness, technical discussion, and reproducibility.

\subsection{Performance Results}

This subsection reports the measured fixed-window and drain-mode outputs. The fixed-window tables describe the final 150-run sweep, while the later drain-mode tables describe deterministic post-fix validation and comparison artifacts.
...
Under this fixed-window measurement policy, the \texttt{XY} hotspot cells cannot be compared with \texttt{DEFT} because the \texttt{XY} side injected zero packets in every hotspot fault-mask cell. T0033 diagnosed those \texttt{XY|hotspot\_3x10} zero-injection cells as measured-window artifacts: with \texttt{-warmup 1000}, early \texttt{XY} traffic had already injected before the measured window and then stalled on topology-incompatible paths. The warm-up-0 diagnostic values from T0033 are kept only as traceability evidence and are not reported as final performance results.

The \texttt{XY} uniform and localized cells injected packets in some seeds, but received zero packets in the measured window. T0033 diagnosed these \texttt{XY|uniform} and \texttt{XY|localized\_40} zero-received cells as standard \texttt{XY} topology incompatibility on \texttt{DEFT\_2\_5D}: cardinal-only \texttt{XY} cannot route unrestricted inter-chiplet traffic through the VL/hub/interposer path required by this topology. Therefore, side-by-side latency comparisons against \texttt{DEFT} are not supported.
...
T0056 and T0053 use opt-in source-cutoff plus drain semantics rather than the T0026 fixed-window policy. These results are artifact-scoped and denominator-safe. They do not update the fixed-window condition metrics above and do not relabel IA-XY as standard \texttt{XY}.

\begin{table}[t]
\centering
\caption{T0056 DeFT Drain-Mode Summary}
\label{tab:t0056_drain_summary}
\begin{tabular}{@{}lr@{}}
\toprule
\textbf{Item} & \textbf{Value} \\
\midrule
...
Fault masks & 5 \\
Simulator cases & 95 \\
\texttt{DEFT} drain-completed pass cases & 95 \\
\texttt{DEFT} timeout or non-100\% cases & 0 \\
All-pairs aggregate rows & 5 \\
All-pairs packets per row & 4032 / 4032 \\
All-pairs flits per row & 32256 / 32256 \\
\bottomrule
\end{tabular}
\end{table}

The five T0056 all-pairs aggregate rows drained after 4088, 4858, 5617, 6239, and 5208 drain cycles for fault masks \texttt{0x0000}, \texttt{0x0001}, \texttt{0x0011}, \texttt{0x0111}, and \texttt{0x1111}, respectively.
...
\textbf{Blocked:} \texttt{XY|hotspot\_3x10} cells injected zero packets for all seeds and fault masks in the measured window; T0033 diagnosed these cells as \texttt{-warmup 1000} measured-window artifacts after early \texttt{XY} traffic injected and then stalled.
...
\textbf{Blocked:} Standard \texttt{XY} is cardinal-only on \texttt{DEFT\_2\_5D} and does not traverse VL/hub/interposer paths required for unrestricted inter-chiplet traffic.

\textbf{Blocked:} The T0056/T0053 drain-mode artifacts are deterministic matrix evidence only; they do not prove universal reachability or universal IA-XY behavior.

\textbf{Blocked:} Standard \texttt{XY} remains cardinal-only. The drain-mode comparison uses \texttt{INTERPOSER\_AWARE\_XY}, which must not be relabeled as standard \texttt{XY}.

\textbf{Blocked:} Stronger result statements, non-empty \texttt{XY} hotspot measurements, latency comparisons, single-direction fault cases, universal rankings, and improvement percentages require separate documented validation or rerun policy.
...
The code and experiment configuration are in the project repository, with the registered Noxim source tree at \path{external/noxim}. The validated Noxim build command is \texttt{./build.sh} from \path{external/noxim} in the documented WSL/Linux environment. The final fixed-window experiment commands, generated LUTs, JSON statistics, manifests, and summaries are preserved under \path{external/noxim/other/generated/t0026_final_sweep_v1/}, \path{external/noxim/other/generated/t0026_final_analysis_v1/}, \path{external/noxim/other/generated/t0027_report_support_v1/}, and \path{external/noxim/other/generated/t0028_final_report_results_v1/}. The post-fix drain-mode artifacts are preserved under \path{external/noxim/other/generated/t0056_deft_post_fix_reachability_v1/} and \path{external/noxim/other/generated/t0053_drain_iaxy_deft_comparison_v1/}; each artifact set includes the relevant commands, fixtures, generated LUTs, logs, summaries, and manifests.
...
The project produced a traceable Noxim extension for \texttt{DEFT\_2\_5D} and a reproducible final-sweep artifact set. The final report-support data confirms that the planned 150-run matrix executed successfully and that generated summary tables cross-check against raw artifacts. The reportable fixed-window results should remain descriptive because the measured data contains blank and partial cells, and the known \texttt{XY} blank-cell behavior is now explained by the T0033 diagnosis.

The post-fix T0056/T0053 drain-mode artifacts add useful matrix-scoped validation evidence: \texttt{DEFT} completed the documented 95-case drain matrix, and IA-XY completed 68 of the 95 matched comparison rows while timing out in 27 rows. The safest conclusion is that the implementation and report-support workflow are in place, while stronger experimental claims require an explicitly scoped follow-up validation policy. Future work should address non-empty hotspot measurements for the standard \texttt{XY} control baseline, IA-XY timeout diagnosis, directional endpoint fault modeling, traffic-profile-specific LUT demand inputs, and validated PARSEC/GEM5 trace support before expanding the report claims.
